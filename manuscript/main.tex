...
\footnote{Note that we should not use bandswidths that within too small of the distances above the Lianjia to the neighborhoods, and this is because our evaluation of the neighborhood is based on the centroid point of the neighborhood, and the store's distance to the centroid point is not necessarily means that it is not close to the neighborhood. Therefore, with distance very close to the neighborhood, we can also think they are approximately close.}
...
One common concern in RDD is the potential for manipulation around the cutoff, which could invalidate the results. To address this, a "donut hole" robustness check can be implemented, where observations close to the cutoff are excluded to ensure that the results are not driven by manipulation or other local irregularities. In this study, we apply a donut hole robustness check to an RDD with a cutoff of 410 meters. Specifically, we sequentially exclude observations within certain distances from the cutoff and re-estimate the treatment effect. The distances considered for exclusion are 100, 125, 150, 175, 200, 225, and 250 meters. This method helps to verify the robustness of the estimated treatment effects to the exclusion of data points near the cutoff.

Table \ref{tab:rd_robust_results} shows the donut hole effect, we exclude the samples within
...
\subsection{Alternative Methods} \label{subsec:alternative_methods}
