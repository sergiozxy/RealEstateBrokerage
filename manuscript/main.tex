...
To check the robustness of the result, we calculate the HHI index for each segmented market which is defined as $HHI = \sum_{i=1}^N (s_i)^2$ where $s_i$ is the market share of firm $i$ expressed as a percentage. Then we estimate the model by separating the samples to those with high HHI and low HHI where HHI is greater than 2000 and less than 2000. The results are shown in Table \ref{tab:sylized_fact_hetero_hhi}. The result indicates that 

Moreover, to check the heterogeneity issue, we also classifies the we also report the results of the robustness check of the dynamic model. The results show that the dynamic model Table \ref{tab:sylized_fact_hetero_mature}
