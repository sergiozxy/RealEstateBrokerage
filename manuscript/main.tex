...
To gather additional characteristic information, we first extracted Point of Interest (POI) data from the AutoNavi map using a web-scraping Python program.\footnote{AutoNavi, a leading mapping application in China with a user base exceeding 700 million, is renowned for its detailed and accurate POI data as well as precise public transportation information. These features underscore AutoNavi's leadership in the digital mapping sector, highlighting its ability to provide unparalleled navigation accuracy and comprehensive urban mobility solutions. Our extracted AutoNavi dataset includes over one million POIs per city annually.} The extracted POIs were then classified into various categories, as detailed in Table \ref{tab
}. This classification is crucial for understanding the urban infrastructure and amenities available in the vicinity of the analyzed properties.

We matched each type of POI to the neighborhood-level data within a 500-meter radius, a distance typically covered by walking and consistent with urban planning standards for accessible urban design. This radius was also specifically selected to reflect the immediate urban environment influencing residential desirability and property values, as the majority of these POIs provide recreational services. Additionally, we integrated geo-informational data into our research, including annual GDP data from \citet{zhao_forecasting_2017}, nighttime lights data from \citet{elvidge_annual_2021}, and air pollution data from \citet{doi:10.1021/acs.est.1c05309}. We generated the centroid of the neighborhood-level data's polygon and then extracted the value from the geo-informational data. By merging these data sources with our research panel, we constructed a comprehensive research sample.
...
In the post-entry periods ($post_1$, $post_2$, and $post_3$), the results exhibit notable variations. The first period after entry ($post_1$) shows no significant changes in any of the metrics analyzed. However, in the subsequent post-entry periods ($post_2$ and $post_3$), there are significant negative effects on both revenue and the number of house tours. These declines suggest a deterioration in Lianjia's performance and customer engagement, likely attributable to the impact of the epidemic, which led to a decrease in overall revenue. Interestingly, Lianjia's entry does not have a significant effect on price concessions but does show a negative impact on the number of house tours during the epidemic periods. This negative effect is also attributed to the epidemic, which reduced the frequency of house tours due to heightened health concerns and mobility restrictions.
...
To verify the robustness of the entry effect, we classified the sample into low and high Herfindahl-Hirschman Index (HHI) groups. The results, presented in Table \ref{tab:entry_effect_robustness}, indicate that the entry effect is more pronounced in the high HHI group, with an impact of 6.8\%, and this effect persists into the subsequent period, increasing to 7.8\%. However, following the epidemic, the effect becomes significantly negative in the low HHI group, suggesting that the entry effect is more adversely affected by epidemic policies in less concentrated markets. Conversely, the effect remains non-significant in the high HHI group. Additionally, Lianjia's entry does not significantly influence price concessions in either group, nor does it significantly affect the number of house tours. However, post-epidemic, the number of house tours shows a significantly negative effect in the low HHI group, indicating a more substantial decline in customer engagement in less concentrated markets.
...
To check the robustness of the result, we also calculated the average number of watching times in the beke's website during the sample periods and estimated the model separately (need to further refine and reformulate).
