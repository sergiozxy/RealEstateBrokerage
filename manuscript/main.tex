...
This weight represents the strength of the connection between the stores. We then calculated the network clustering effect for each neighborhood using the following formula:

\begin{equation}
  \text{Clustering Effect}_{\text{neighborhood}} =  \label{eq:clustering_effect_for_neighborhood}
\end{equation}

where $n$ represents the number of stores connected to the neighborhood directly and indirectly, and $m$ represents the number of shared transactions between connected stores. This approach allows us to quantify the local clustering effect within the segmented market and assess its impact on neighborhoods.

This connection is in line with the reality in that firstly, cooperation between stores to complete a transaction is very common, and more stores within the segmented market can contributed more to the transaction as more efforts can be jointly made between different agents. Secondly, the influence of the netowrk decays from subconnected nodes and this is due to the decreasing benefits by connecting other sub-connecting.
