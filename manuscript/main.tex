...
\begin{equation}
    Y_{it} = \beta_0 +  \beta_1 pre_2 + \beta_2 entry + \beta_3 post_1 + \beta_4 post_2 + \beta_5 post_3 + \bf{\alpha} \bf{X}_{it} + \tau_{it} + \mu_i + \epsilon_{it}.   \label{eq:entry_effect}
\end{equation}
...
We then calculated the network clustering effect for each neighborhood using Breadth First Search Algorithm (BFS) described in the Algorithm \ref{alg:clustering_effect}:
...
% need to refine
This approach allows us to quantify the local clustering effect within the segmented market and assess its impact on neighborhoods. This connection is in line with the reality in that firstly, cooperation between stores to complete a transaction is very common, and more stores within the segmented market can contributed more to the transaction as more efforts can be jointly made between different agents. Secondly, the influence of the netowrk decays from subconnected nodes and this is due to the decreasing benefits by connecting other sub-connecting.




Following the Lianjia's platform consolidation strategy, Lianjia decided to consolidate downstream brokerages to form a larger clustering within the segmented market and increase. By attracting other brokerage's into the network will initially make the brokreage's profit down because attracting other people in the platform meaning the market share would be decreased due to the intense competition. However, after the setup of the platform, the market will be more productive and more competitive to occupying a larger share of the whole network. This mechanism is more likely to get better control over the listing and people are more willing to list their house on this platform. Then according to the this would be resulted in a winners-take-all market.
...
\begin{equation}
  \begin{aligned}
    Q_y(\xi \mid X) & = \beta_0 + \beta_1(\xi) \text{pre\_treatment}_{it} + \beta_2(\xi) \text{treatment}_{it} + \sum_{i=3}^5 \beta_i(\xi) \text{post\_treatment}_{it} \\
